...
/*
|--------------------------------------------------------------------------
| Dashboard Routes (Admin & Mahasiswa, dipisah)
|--------------------------------------------------------------------------
*/
...
/*
|--------------------------------------------------------------------------
| ADMIN ONLY: Manajemen Data Master (Dosen, Mahasiswa, Matakuliah, Jadwal)
|--------------------------------------------------------------------------
*/
Route::middleware(['auth', 'role:admin'])->group(function () {
    ...
    // JADWAL (kelola penuh oleh admin)
    ...
    // Export KRS (Admin) - harus didaftarkan SEBELUM route {id} agar "export" tidak tertangkap sebagai id
    Route::get('/krs/export/pdf', [ExportController::class, 'krsAllPdf'])->name('krs.export.pdf');
    Route::get('/krs/export/excel', [ExportController::class, 'krsAllExcel'])->name('krs.export.excel');

    // KRS (admin hanya melihat & menghapus jika perlu, tidak input atas nama mahasiswa)
...
/*
|--------------------------------------------------------------------------
| MAHASISWA ONLY: KRS (ambil/drop) & Lihat Jadwal
|--------------------------------------------------------------------------
*/
Route::middleware(['auth', 'role:mahasiswa'])->group(function () {

    // KRS milik sendiri
    Route::get('/krs-saya', [MahasiswaKrsController::class, 'index'])->name('krs.mahasiswa.index');
    Route::get('/krs-saya/create', [MahasiswaKrsController::class, 'create'])->name('krs.mahasiswa.create');
    Route::post('/krs-saya/store', [MahasiswaKrsController::class, 'store'])->name('krs.mahasiswa.store');
    Route::get('/krs-saya/export/pdf', [ExportController::class, 'krsSayaPdf'])->name('krs.mahasiswa.export.pdf');
    Route::delete('/krs-saya/{id}/delete', [MahasiswaKrsController::class, 'destroy'])->name('krs.mahasiswa.delete');

    // Lihat jadwal (read-only)
    Route::get('/jadwal-saya', [JadwalLihatController::class, 'index'])->name('jadwal.mahasiswa.index');
});
